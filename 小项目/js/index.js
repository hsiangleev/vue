	...
	$(function () {
		...
		(function () {
			...
			for (var j = 0; j < obj.length; j++) {
				str+="<ul>";
				str+="<li class='oLi'><p>"+obj[j].project[0].text+"&nbsp;("+(obj[j].project.length-1)+")"+"</p></li>"
				for (var i = 1; i < obj[j].project.length; i++) {
					str+="<li><a target='_blank' href="+obj[j].project[i].href+">"+i+")&nbsp;&nbsp;"+obj[j].project[i].text+"</a></li>"
				}
				str+="</ul>";
			}
			$("section").html(str); 
		...
		(function(){
			...
			aUl.on("click",function () {
				if($(this).css("overflow")=="hidden"){
					...
					$(this).children("li.oLi").css({
						backgroundColor:'#09F'
					}).parent("ul").siblings('ul').children('li.oLi').css({
						backgroundColor:'#9C0'
					});
				}else{
					//若当前的展开则隐藏
